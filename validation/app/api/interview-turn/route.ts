...
// Keep this in sync with Product/interview-bot-prompt.md — that file is the
// source of truth for the prompt; this is the copy the server actually
// sends, kept in-repo so the route has no dependency on a path outside the
// Next.js app.
const SYSTEM_PROMPT = `You are a conversational discovery-interview bot embedded in an internal PM
...
3. Follow-up depth is your live judgment call, not a pre-set tier.
   ...
   - When you do probe, ask ONE follow-up that digs toward the actual
     ...
     "How often does that come up?", "What would it mean for you if this
     were fixed?") — never just "can you say more," and never repeat the
     same follow-up phrasing if you end up asking more than one.
   - You may fire at most 2 follow-ups on any single question, regardless
     of how thin the answers keep coming. Re-evaluate after each one: stop
     as soon as an answer has real substance, or once you've used both,
     whichever comes first — then move on either way.
...
4. Handle off-script questions from the customer. If the customer asks
   ...
   that for the PM." Do not guess or invent an
   answer.
...
- close_session: emitted only when screen_complete would otherwise fire on
  the last screen (no next screen to advance to) — closes the session
  directly instead of pointing at a "Next" button that has nowhere to go.`;
...
const VALID_ACTIONS = new Set<BotAction>([
  "ask_question",
  "ask_followup",
  "answer_orientation",
  "flag_for_pm",
...
export async function POST(request: Request) {
  ...
  const body = (await request.json()) as InterviewTurnRequest;
  ...
  const contextPayload = {
    ...
    is_last_screen: body.isLastScreen,
    conversation_history: body.history,
    current_position: {
      question_id: body.currentQuestionId,
      followup_turn_count: body.followupTurnCount,
    },
  ...
  try {
    ...
    console.error("Interview turn call failed", err);
  ...
  try {
    ...
    console.error("Failed to parse interview bot response as JSON:", raw);
    return Response.json(
  ...
  if (
    typeof parsed.message_to_customer !== "string" ||
